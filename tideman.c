...
// Vote function which updates ranks
bool vote(int rank, string name, int ranks[])
{
    for (int i = 0; i < candidate_count; i++)
    {
...
// I use quick sort as the sorting algorithm
void sort_pairs(void)
{
    qsort(pairs, pair_count, sizeof(pair), comparator);
}

//Function to lock the pairs
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        if (i == 0 || pairs[i].loser != pairs[0].winner)
        {
...
void print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        // Prints the source of the graph
